...
{
    // Bells: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // (old) Bells: every 4 hours
static const int64 nTargetTimespanNEW = 150 ; // Bells: every 2.5 minutes

static const int64 nTargetSpacing = 60; // (old) Bells: 1 minutes
static const int64 nTargetSpacingNEW = 150; // Bells: 2.5 minutes

static const int64 nInterval = nTargetTimespan / nTargetSpacing;

static const int64 nChangeTarget = 203000; // Bells: Hardfork occuring at block number 203000
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
		if(nBestHeight+1 >= nChangeTarget)
		{
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    bool fNewDifficultyProtocol = (nHeight >= nChangeTarget);
    ...
    if (fNewDifficultyProtocol) {
		...
		retargetSpacing = nTargetSpacingNEW;
	...
	{
		if (nActualTimespan < retargetTimespan/8)
			nActualTimespan = retargetTimespan/8;
		if (nActualTimespan > retargetTimespan*4)
			nActualTimespan = retargetTimespan*4;
	}
	else 
	{
		if (nActualTimespan < retargetTimespan/16)
			nActualTimespan = retargetTimespan/16;
		if (nActualTimespan > retargetTimespan*4)
			nActualTimespan = retargetTimespan*4;
	}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bells: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bells: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // Bells: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BellsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BellsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BellsMiner(CWallet *pwallet)
{
    printf("BellsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Bells-miner");
    ...
    try { loop {
        ...
        printf("Running BellsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BellsMiner, pwallet));
}
